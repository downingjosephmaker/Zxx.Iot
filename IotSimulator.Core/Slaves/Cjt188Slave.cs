...
using IotSimulator.Core.Generators;
...
namespace IotSimulator.Core.Slaves
{
    ...
    public sealed class Cjt188Slave : IProtocolSlave
    {
        ...
        public Cjt188Slave(DeviceModel model)
        {
            Address = model.Address;
            _meterType = ParseByte(model.MeterType, 0x10);  // 默认0x10冷水表
            _addrBcd = BuildAddressBcd(model.Address, 14);
            foreach (var pm in model.Points)
            {

...
namespace CenBoCommon.Zxx
{
    ...
    {
        public static List<T> GetListBy<T>(this ActionPara model, List<T> oldlist)
        {
            var oldpage = model.page;
            var oldpagesize = model.pagesize;
            try
            {
                model.page = 0;
                model.pagesize = 0;
                var dataitem = model.GetListByPage(oldlist);
                return dataitem.Item1;
            }
            finally
            {
                model.page = oldpage;
                model.pagesize = oldpagesize;
            }
        }

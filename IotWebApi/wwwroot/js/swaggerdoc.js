...
function LoadExportApiWordBtn() {
    //$(".download-url-wrapper").height(240);//文档介绍区域高度
    //$(".topbar").height(35);
    var btnExport = "<div class='selectBox' style='margin: 0;padding: 0;max-height:40px;'>" +
        ...
        "</div >" +
        "</div >";
    //download-url-wrapper这个元素是swagger后期动态渲染出来的，所有这里要加个循环判断。
    //第一次进来如果有这个class直接加载按钮退出
    if ($("*").hasClass("download-url-wrapper")) {
        if ($(".selectBox").length == 0) {
            $(".download-url-wrapper").prepend(btnExport);
            $(".select-label span").text('选择版本');
            $(".topbar-wrapper .link").hide();
        }
        return;
    }
    //没有元素等待元素出现在加载按钮
...
function ExportApiWord(type) {
    ...
    var docname = $("#select option:selected").text();
    ...
    xhr.onload = function () {
        ...
        if (this.status === 200) {
            ...
            reader.onload = function (e) {
                ...
                a.download = docname + '（' + version + '）' + exten;

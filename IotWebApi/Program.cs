...
// 告警屏蔽引擎(告警产生之后、入库通知之前过滤,六scope×三模式×三动作)
builder.Services.AddSingleton<AlarmMaskService>();
...
// 规则联动引擎(§10.1:扁平触发-条件-动作,四型触发/白名单命令/虚拟点位/通知/Webhook)
...
// TCP南向上报监听器(复用TcpServerChannel连接管理,换行定界拆帧,默认端口0=不启用)

...
namespace IotWebApi.Controllers
{
    ...
    public class EventAlarmDbController : ControllerBaseApi
    {
        ...
        public AlarmChart GetAlarmAllDataChart(int alarmselecttype = 1)
        {
            ...
            Expression<Func<EventAlarm, bool>> expression = t => t.TenantId == optmdl.TenantId;
            if (alarmselecttype == 1) expression = t => t.TenantId == optmdl.TenantId && t.CheckResult == "未处理";
            ...
            foreach (string key in AlarmTypeList)
            {
                ...
                Expression<Func<EventAlarm, bool>> typeExpression = t => t.TenantId == optmdl.TenantId && t.AlarmType == key;
                if (alarmselecttype == 1) expression = t => t.TenantId == optmdl.TenantId && t.AlarmType == key && t.CheckResult == "未处理";
            ...
            foreach (string key in AlarmGradeList)
            {
                ...
                Expression<Func<EventAlarm, bool>> typeExpression = t => t.TenantId == optmdl.TenantId && t.AlarmGrade == key;
                if (alarmselecttype == 1) expression = t => t.TenantId == optmdl.TenantId && t.AlarmGrade == key && t.CheckResult == "未处理";
                var datavaluetype = SysCommonDAO<EventAlarm>.Instance.GetListCount(typeExpression);
                if (datavaluetype > 0)
                {
                    chartChild.ChartY.Add(datavaluetype.ToString());
        ...
        public string GetAlarmAnalysisTop()
        {
            ...
            var dayvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= daysnowid && t.TenantId == optmdl.TenantId && t.EventType != "离线");
            if (dayvalue > 0)
            {
                dayva = dayvalue;
                json["今日报警数"] = dayva;
            ...
            var yesvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= yessnowid && t.SnowId < daysnowid && t.TenantId == optmdl.TenantId && t.EventType != "离线");
            if (yesvalue > 0)
            {
                yesva = yesvalue;
                json["昨日报警数"] = yesva;
            ...
            var daymonthvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= daymonthsnowid && t.TenantId == optmdl.TenantId && t.EventType != "离线");
            if (daymonthvalue > 0)
            {
            ...
            var yesmonthvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= yesmonthsnowid && t.SnowId < daymonthsnowid && t.TenantId == optmdl.TenantId && t.EventType != "离线");
            if (yesmonthvalue > 0)
            {
            ...
            var dayyearvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= dayyearsnowid && t.TenantId == optmdl.TenantId && t.EventType != "离线");
            if (dayyearvalue > 0)
            {
            ...
            var yesyearvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= yesyearsnowid && t.SnowId < dayyearsnowid && t.TenantId == optmdl.TenantId && t.EventType != "离线");
            if (yesyearvalue > 0)
            {
        ...
        public List<RestfulAlarmAnalysisTwo> GetAlarmAnalysisTwo(DateTime starttime, DateTime endtime, int alarmselecttype)
        {
            ...
            if (alarmselecttype == 1)
            {
                ...
                foreach (string key in AlarmTypeList)
                {
                    ...
                    var dayvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= min && t.SnowId <= max && t.TenantId == optmdl.TenantId && t.AlarmType == key && t.EventType != "离线");
                    if (dayvalue > 0)
                    {
            ...
            else if (alarmselecttype == 2)
            {
                ...
                foreach (string key in AlarmGradeList)
                {
                    ...
                    var dayvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= min && t.SnowId <= max && t.TenantId == optmdl.TenantId && t.AlarmGrade == key && t.EventType != "离线");
                    if (dayvalue > 0)
                    {
                        one.AlarmAllCount = dayvalue;
                    }
        ...
        public DataChart GetAlarmAnalysisThree(DateTime starttime, DateTime endtime)
        {
            ...
            for (int i = 0; i < days; i++)
            {
                ...
                var dayvalue = SysCommonDAO<EventAlarm>.Instance.GetListCount(t => t.SnowId >= min && t.SnowId <= max && t.TenantId == optmdl.TenantId && t.EventType != "离线");
                if (dayvalue > 0)
                {
